...
//    findJobDescriptionGivenJobId2(jobId).getOrElse("")
    findJobDescriptionGivenJobId2(jobId).fold(s"Job with id $jobId does not exist")(identity)
...
//    findHumanById(humanId).map(h => h.optJobId).flatten
    findHumanById(humanId).flatMap(h => h.optJobId)
