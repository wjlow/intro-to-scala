...
object LogParser {
  ...
  trait LogMessage
  case class KnownLog(logLevel: LogLevel,
                      timestamp: Timestamp,
                      message: String)
    extends LogMessage
 ...
 /**
  ...
  def parseLog(str: String): LogMessage = {
    ...
    val optLog: Option[LogMessage] = fields match {
      case List("I", timestampStr, message) =>
        parseIntOption(timestampStr)
          .map(timestamp => KnownLog(Info, timestamp, message))
      case List("W", timestampStr, message) =>
        parseIntOption(timestampStr)
          .map(timestamp => KnownLog(Warning, timestamp, message))
      case List("E", sevStr, timestampStr, message) => for {
  ...
  def parseLogFile(fileContent: String): List[LogMessage] =
    fileContent.lines.toList.map(parseLog)
  ...
  /**
    ...
    * = List()
    **/
  def getErrorsOverSeverity(logs: List[LogMessage], severity: Int)
  : List[LogMessage] = {
    logs.filter{
...
//    logs.collect {
//      case l@KnownLog(Error(s), _, _) if s > severity => l
//    }
  ...
  def showLogMessage(log: LogMessage): String = log match {
    ...
    case KnownLog(level, timestamp, message) =>
      s"$level ($timestamp) $message"
  ...
  def showErrorsOverSeverity(fileContent: String, severity: Int): List[String] = {
    ...
    val overSeverity = getErrorsOverSeverity(messages, severity)
    overSeverity.map(showLogMessage)
  }
